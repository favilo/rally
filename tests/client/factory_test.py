...
TEST_CONFIG = NodeConfig("http", "localhost", 9200, headers=HttpHeaders({"x-elastic-product": "Elasticsearch"}))


def _api_error(status, message):
    return elasticsearch.ApiError(
        message,
        ApiResponseMeta(status=status, http_version="1.1", headers=HttpHeaders({}), duration=0.0, node=TEST_CONFIG),
        None,
    )

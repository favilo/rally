...
class FileSource:
    ...
    def readlines(self, num_lines: int) -> List[bytes]:
        assert self.f is not None
        lines = []
        f = self.f
    ...
    def __exit__(
        self, exc_type: Optional[Type[BaseException]], exc: Optional[BaseException], traceback: Optional[types.TracebackType]
    ) -> Literal[False]:
        self.close()
        return False

    def __str__(self, *args: Collection[Any], **kwargs: Mapping[Any, Any]) -> str:
...
class DictStringFileSourceFactory:
    ...
    def __init__(self, name_to_contents: Mapping[str, List[str]]):
...
class StringAsFileSource:
    ...
    def __init__(self, contents: List[str], mode: str, encoding: str = "utf-8"):
        """
        ...
        """
        self.contents: Optional[List[str]] = contents
    ...
    def readlines(self, num_lines: int) -> List[str]:
        lines = []
        for _ in range(num_lines):
            line = self.readline()
            if len(line) == 0:
...
def decompress(zip_name: str, target_directory: str) -> None:
    ...
    * zst
    ...
    decomplressor_lib: Union[Callable, Type[ZstAdapter]]
    if extension == ".zip":
        ...
        decompressor_lib_gz = gzip.open
        _do_decompress_manually(target_directory, zip_name, decompressor_args, decompressor_lib_gz)
...
def _do_decompress_manually(
    target_directory: str, filename: str, decompressor_args: List[str], decompressor_lib: Callable[[str], Any]
) -> None:
...
def _do_decompress(target_directory: str, compressed_file: Union[zipfile.ZipFile, tarfile.TarFile]) -> None:
    try:
        ...
        if isinstance(compressed_file, zipfile.ZipFile):
            raise RuntimeError(f"Could not decompress provided archive [{compressed_file.filename!r}]")
        elif isinstance(compressed_file, tarfile.TarFile):
            raise RuntimeError(f"Could not decompress provided archive [{compressed_file.name!r}]")
    finally:
        compressed_file.close()
...
class FileOffsetTable:
    ...
    def add_offset(self, line_number: int, offset: int) -> None:
        ...
        """
        assert self.offset_file is not None
